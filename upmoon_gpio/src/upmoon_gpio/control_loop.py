...
from upmoon_gpio.lib.Stepper import Stepper
import rospy
...
class ControlLoop:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("motors/wheel/l", Float64MultiArray, self.callback0)
        rospy.Subscriber("motors/wheel/r", Float64MultiArray, self.callback1)
        rospy.Subscriber("motors/ankle/l", Float64MultiArray, self.callback2)
        rospy.Subscriber("motors/ankle/r", Float64MultiArray, self.callback3)
        ...
        #art_disable_pins = [[37,37,37], [37,37,37]]
        ...
        #            continue
        self.art_motors.append(Stepper(37, 7, 11))
    ...
    def control(self):
        while not rospy.is_shutdown():
            ...
            if self.ankle_update: # If the angle is updated by ROS, update the Stepper angles
                self.anke_update = False
                print('Updating ankles')
                for i in range(1):
                    print('Seting ankle to', self.ankle_left.data)
                    self.art_motors[i].setAngle(int(self.ankle_left.data[i]))
                    #self.art_motors[3+i].setAngle(int(self.ankle_right.data[i])) #Yeah indexing is funny...
